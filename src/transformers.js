...
export function makePlaylistFromCatalog(catalog) {
  return Object.entries(catalog).flatMap(([artistSlug, artist]) =>
    Object.entries(artist.albums).flatMap(([albumSlug, album]) =>
      makePlaylistFromAlbumTracks(album.tracks, { albumSlug, album: album.name }, { artistSlug, artist: artist.name }),
    ),
  );
}
...
export function makeAlbumsFromArtist(artist) {
  return Object.entries(artist.albums).map(([k, v]) => ({ ...v, slug: k }));
}
